...
def build_search_graph(search_node: SearchNode, enrich_node: EnrichNode | None = None) -> Any:
...
async def run_search_graph(
    ...
    *,
    ...
    should_build_default_enrich = search_node is None and enrich_node is None
    active_enrich_node = (
        create_default_enrich_node()
        if should_build_default_enrich
        else enrich_node
    )
    app = build_search_graph(active_node, enrich_node=active_enrich_node)
